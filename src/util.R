...
createCNmatrix <- function(CNbins,
  ...
  if (wholegenome == TRUE) {
    genome <- as.data.table(getBins(binsize = CNbins$end[1] - CNbins$start[1] +
                                      1))
    CNbins <- lapply(unique(CNbins$cell_id), function(x)
      merge(
        genome,
        CNbins[cell_id == x],
        on = c("chr", "start", "end"),
  ...
  if (fillnaplot == TRUE) {
    ...
    narows <- apply(narows, 1, function(x)
      sum(is.na(x))) / length(colnames)
  ...
  rownames(cnmatrix) <- paste(cnmatrix$chr,
                              as.integer(cnmatrix$start),
                              as.integer(cnmatrix$end),
                              sep = "_")
...
#' @export
getBins <- function(chrom.lengths = hg19_chrlength,
                    binsize = 1e6,
                    chromosomes = NULL) {
  if (!requireNamespace("GenomeInfoDb", quietly = TRUE)) {
    stop(
      "Package \"GenomeInfoDb\" needed for this function to work. Please install it.",
      call. = FALSE
    )
  }
  ...
  if (length(chroms2use) == 0) {
    ...
    stop("Could not find length information for any of the specified chromosomes: ",
         chrstring)
  }
  ...
  if (length(diff) > 0) {
    ...
    warning("Could not find length information for the following chromosomes: ",
            diffs)
  }
  ...
  if (length(skipped.chroms) > 0) {
    warning(
      "The following chromosomes are smaller than binsize ",
      binsize,
      ": ",
...
snv_states <- function(SNV, CNbins) {
  ...
  mappedSNVs <- CN[SNV, on = .(chry == chr, cell_idy == cell_id, starty < start, endy > start)]
...
map_to_segments <- function(regions, segments) {
  ...
  mapped_segs <- as.data.table(regions)[as.data.table(segments), on = .(chr == chr, start > start, start < end)] %>%
    .[, start := pos] %>%
    .[, pos := NULL] %>%
    .[, start.1 := NULL] %>%
    na.omit()
...
create_segments <- function(CNbins, field = "state") {
  newsegs <- CNbins %>%
    ...
    .[, list(start = min(start), end = max(end)), by = .(cell_id, chr, get(field), rlid)] %>%
    .[order(cell_id, chr, start)] %>%
...
create_cntransitions <- function(CNbins,
  ...
  newsegs <- CNbins %>%
    ...
    .[order(cell_id, chr, start)] %>%
    .[, rlid := data.table::rleid(get(field)), by = cell_id] %>%
    .[, list(start = min(start), end = min(start) + binsize - 1), by = .(cell_id, chr, get(field), rlid)] %>%
    .[order(cell_id, chr, start)] %>%
  ...
  if (add_orientation) {
    newsegs <- CNbins %>%
      ...
      .[, list(start = min(start),
               end = min(start) + binsize - 1), by = .(cell_id, chr, state, rlid)] %>%
      .[order(cell_id, chr, start)] %>%
      dplyr::group_by(cell_id, chr) %>%
      dplyr::mutate(rown = dplyr::row_number()) %>%
...
qc_summary <- function(cn) {
  ...
  message(paste0(
    "Average distance from median to expected BAF = ",
    round(summary_distance, 4)
  ))
...
qc_per_cell <- function(cn) {
  ...
  pl <- cn %>% as.data.table() %>% .[!chr %in% c("X", "Y"), list(ploidy = Mode(state),
                                                                 totalhapcounts = sum(totalcounts)), by = "cell_id"]
...
per_arm_baf_mat <- function(haps,
  ...
  if (mergelowcounts & is.null(arms)) {
    ...
    message(paste0("Only using specific chromosome arms: "),
            paste0(arms, collapse = ", "))
    baf <- haps %>%
      dplyr::filter(chr != "Y") %>%
      dplyr::mutate(arm = coord_to_arm(chr, start, mergesmallarms = FALSE)) %>%
      dplyr::mutate(chrarm = paste0(chr, arm)) %>%
  ...
  warning(
    paste0(
      "The following chromosomes have on average ",
      mincounts,
      " or fewer counts: ",
      paste(counts$chrarm, collapse = ", ")
    )
  )
...
filter_segments <- function(segs, binwidth = 5e6) {
  ...
  segs <- dplyr::left_join(
    segs,
    hg19chrom_coordinates %>% dplyr::filter(arm == "") %>% dplyr::mutate(start = start + 1),
    by = "chr"
  ) %>%
    dplyr::group_by(chr) %>%
    dplyr::mutate(
      start = ifelse(dplyr::row_number() == 1, start.y, start.x),
...
per_segment_baf_mat <- function(haps,
  ...
  mapped_segs$segid <- paste(mapped_segs$chr,
                             mapped_segs$seg_start,
                             mapped_segs$seg_end,
                             sep = "_")
  ...
  warning(
    paste0(
      "The following segments have on average ",
      mincounts,
      " or fewer counts: ",
      paste(counts$segid, collapse = ", ")
    )
  )
...
createBAFassay <- function(seur, rna_ascn, ref = "hg19") {
  ...
  x <- tidyr::pivot_wider(
    rna_ascn$hscn %>%
      ...
      dplyr::mutate(segid = paste0(
        "BAF-", stringr::str_replace_all(segid, "_", "-")
      )),
    names_from = "segid",
    values_from = c("BAF")
  ) %>%
  ...
  clonesdf <- dplyr::distinct(rna_ascn$clusters, cell_id, clone_id) %>%
    as.data.frame() %>%
    dplyr::mutate(clone_id = ifelse(clone_id == "" |
                                      is.na(clone_id), NA, clone_id)) %>%
  ...
  if (!is.null(rna_ascn$haplotypecounts)) {
    ...
    snps_to_genes <- map_to_segments(
      rna_ascn$haplotypecounts,
...
singletons <- function(cn, field) {
  ...
  x <- CNbins %>%
    ...
    dplyr::mutate(singleton = changepoint_forward &
                    changepoint_reverse) %>%
...
filtercn <- function(cn,
  ...
  qcnew <- dplyr::filter(
    cn$qc_per_cell,
    average_distance <= average_distance_filt,
...
#' @export
format_tree_labels <- function(tree,
                               removeloci = TRUE,
                               internal_node_string = 'locus|internal') {
  if (removeloci) {
